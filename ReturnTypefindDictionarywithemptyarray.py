...
def find_dicts_with_empty_choiceCategories(nested_dict):
    result = []
    if isinstance(nested_dict, dict):
        if nested_dict.get("choiceCategories") == []:
            result.append(nested_dict)
        for value in nested_dict.values():
            result.extend(find_dicts_with_empty_choiceCategories(value))
    elif isinstance(nested_dict, list):
        for item in nested_dict:
            result.extend(find_dicts_with_empty_choiceCategories(item))
    return result
